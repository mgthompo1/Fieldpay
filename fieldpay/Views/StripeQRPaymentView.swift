...
struct StripeQRPaymentView: View {
    ...
    @State private var checkoutSession: StripeCheckoutSession?
    ...
    var body: some View {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            HStack {
                                Text("Status:")
                                    ...
                                    .foregroundColor(.secondary)
                                Spacer()
                                Text(paymentStatus.isEmpty ? session.status.capitalized : paymentStatus)
                                    .font(.caption)
                                    .fontWeight(.medium)
                                    .foregroundColor(getStatusColor(session.status))
                            }
                        }
                        .padding()
                        .background(Color(.systemGray6))
        ...
        .onAppear {
            print("Debug: StripeQRPaymentView - View appeared")
            print("Debug: StripeQRPaymentView - Amount: \(amount)")
            print("Debug: StripeQRPaymentView - Customer: \(customer.name)")
            createCheckoutSession()
        }
        .onDisappear {
    ...
    private func createCheckoutSession() {
        ...
        Task {
            do {
                ...
                let session = try await stripeManager.createCheckoutSession(
                    amount: amount,
                    currency: "usd",
                ...
                await MainActor.run {
                    ...
                    // Generate QR code
                    if let checkoutUrl = session.url {
                        self.qrCodeImage = generateQRCode(from: checkoutUrl)
                ...
                await MainActor.run {
                    ...
                    print("Debug: StripeQRPaymentView - Error creating session: \(error)")
                }
            }
        }
    }
    ...
    private func startSessionMonitoring(sessionId: String) {
        ...
        print("Debug: StripeQRPaymentView - Starting session monitoring for: \(sessionId)")
        
        // Start polling the session status
        sessionPollingTask = Task {
            while !Task.isCancelled && isMonitoringSession {
                do {
                    let sessionStatus = try await stripeManager.checkCheckoutSessionStatus(sessionId: sessionId)
                    
                    await MainActor.run {
                        paymentStatus = sessionStatus.paymentStatus.capitalized
                        
                        // Check if payment is completed
                        if sessionStatus.isCompleted {
                            print("Debug: StripeQRPaymentView - Payment successful!")
                        ...
                        if sessionStatus.isExpired {
                            print("Debug: StripeQRPaymentView - Session expired")
                            handlePaymentFailure("Payment session expired")
                            isMonitoringSession = false
                            return
                        }
                    }
                    ...
                    print("Debug: StripeQRPaymentView - Session monitoring error: \(error)")
    ...
    private func handlePaymentSuccess(_ sessionStatus: StripeCheckoutSessionStatus) {
        print("Debug: StripeQRPaymentView - Processing successful payment")
        
        let payment = Payment(
            amount: amount,
            currency: sessionStatus.currency?.uppercased() ?? "USD",
            status: .succeeded,
            paymentMethod: .manualCard,
            customerId: customer.id,
            invoiceId: nil,
            description: "Stripe Checkout Payment",
            stripePaymentIntentId: sessionStatus.paymentIntentId,
            netSuitePaymentId: nil,
            createdDate: Date(),
            processedDate: Date(),
            failureReason: nil
        )
    ...
    private func handlePaymentFailure(_ errorMessage: String) {
        print("Debug: StripeQRPaymentView - Processing failed payment: \(errorMessage)")
        
        let error = NSError(
            domain: "StripePayment",
            code: 999,
            userInfo: [
